...
Last Updated (UTC): 2026-03-12T22:02:39.161239+00:00
...
Session 25 — Full clean weekly pipeline run (18 steps) end-to-end. All layers pass.

- Fixed stray ``` markdown artifact in run_weekly_update.py line 186 (SyntaxError if run directly).
- Fixed patch_source_canonicalization_2026.py: updated CANONICAL_SOURCE_IDS from 11 to 14
  (added bleacherreport_2026/27, combine_ranks_2026/28, nflcom_2026/30). Added ALIAS_MAP
  entry for combine_2026 (source_id=31) -> combine_ranks_2026 (28). Ingest creates combine_2026
  from staged combine_2026.csv on each run; canonicalization now correctly deactivates it.
- Pipeline: EXIT 0, "OK: weekly pipeline completed successfully."
- Doctor: PASSED (sources=31, sources_active=14, source_rankings=33744).
- Snapshot integrity: PASSED (snapshot_id=3, rows=995, coverage=995, confidence=995).
- model_outputs: 999 rows (was 615 — grew with 14 active sources including nflcom_2026).
- Consensus: 995 rows (was 849).
- Tag triggers: 0 new pending recs, 65 idempotent. status: accepted=55, dismissed=22, pending=0.
- NGS deactivated confirmed: ngs_2026 (source_id=29) is_active=0.
- Active sources: 14 canonical.

Prior sessions: 24 (tag trigger engine), 23b (source key correction), 23 (combine+NGS ingest),
...
- APEX top-50 re-score with positional archetype libraries. Current scores use pre-archetype
  trait vectors from Session 4. Run: python -m scripts.run_apex_scoring_2026 --batch top50 --force --apply 1
  Verify all prospect_ids in TOP50_POSITION_OVERRIDES and ARCHETYPE_OVERRIDES before running.
...
22. **APEX top-50 re-score with positional archetype libraries** ← NEXT
23. RAS re-ingest after pro days complete — run ingest_ras_2026.py with updated file
