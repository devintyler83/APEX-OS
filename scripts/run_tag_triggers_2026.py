...
from scripts.divergence_triage_helper import triage_divergence
...
# ---------------------------------------------------------------------------
# Divergence display helper
# ---------------------------------------------------------------------------

def _format_triggered_value(rule_id: int, ctx: dict[str, Any], triggered_value: str) -> str:
    """
    Preserve evaluator output for all non-divergence rules.
    For Divergence Alert rules (5,6), replace generic numeric triggered_value
    with a more useful one-line triage note from divergence_triage_helper.
    """
    if rule_id not in _DIVERGENCE_RULE_IDS:
        return triggered_value

    delta = ctx.get("apex_consensus_divergence")
    if delta is None:
        return triggered_value

    fm_codes_raw = ctx.get("_fm_codes") or []
    if isinstance(fm_codes_raw, str):
        fm_codes = [x.strip() for x in fm_codes_raw.split(",") if x.strip()]
    else:
        fm_codes = list(fm_codes_raw)

    suggestion = triage_divergence(
        prospect_name=ctx.get("_display_name", ""),
        position_group=ctx.get("_position_group", ""),
        archetype_code=ctx.get("_matched_archetype", ""),
        fm_codes=fm_codes,
        delta=delta,
    )
    note = suggestion.get("note", "").strip()
    return note or triggered_value
...
def _load_rules(conn) -> list[dict]:
    ...
    for row in rows:
        if not row["rule_expression"]:
            print(
                f"  [WARN] Empty rule_expression for rule_id={row['rule_id']} "
...
def _load_prospect_data(
    conn,
    season_id: int,
    prospect_id_filter: int | None = None,
) -> list[dict]:
    """
    ...
    for row in rows:
        ...
        fm_codes = [
            x for x in [row["failure_mode_primary"], row["failure_mode_secondary"]]
...
def evaluate(
    ...
    for ctx in prospects:
        ...
        for rule in rules:
            ...
            if isinstance(triggered_value, str) and triggered_value.startswith("ERROR:"):
                print(
                    f"  [WARN] rule_id={rule_id} ({rule_name}) pid={ctx['_prospect_id']}: "
                    f"{triggered_value}"
                )
                continue

            if passed:
                formatted_tv = _format_triggered_value(rule_id, ctx, triggered_value)
                fired.append((rule, ctx, formatted_tv))
    ...
    if not apply:
        ...
        print()
        pid_to_ctx: dict[int, dict[str, Any]] = {}
    ...
    for rule, ctx, tv in fired:
        ...
        exists = conn.execute(
            """
            SELECT 1
            FROM prospect_tag_recommendations
    ...
    tag_recs = conn.execute(
        ...
        JOIN tag_definitions td
          ON td.tag_def_id = ptr.tag_def_id
